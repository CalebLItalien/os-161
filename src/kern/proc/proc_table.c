...
int add_proc(int pid, struct proc_table *pt, struct proc * p){
    ...
    */
    ...
    if (proc_table_full(pt)) { return -1; } // placeholder, replace with proper error
...
int get_proc(int pid, struct proc_table *pt, struct proc **p){
    ...
    KASSERT(pt != NULL);
    if (!valid_pid(pid)) { return -1; } // placeholder, replace with proper error

    spinlock_acquire(&pt -> pt_lock);

    *p = pt->proc_table_map[1][pid].procPtr;

    if (p == NULL) { return -1; } // placeholder, replace with proper error
...
int get_available_pid(struct proc_table *pt){
    ...
    */
    ...
    spinlock_release(&pt->pt_lock);
    return -1; // placeholder, replace with proper error
}
...
struct proc * remove_process(struct proc_table *pt, int pid){
    ...
    */
    KASSERT(pt != NULL);
    if (!valid_pid(pid)){ return NULL; } // you know what i'm gonna say

    spinlock_acquire(&pt -> pt_lock); 

    struct proc * removed_proc = pt -> proc_table_map[1][pid].procPtr;
    if (proc_to_remove != NULL) {
